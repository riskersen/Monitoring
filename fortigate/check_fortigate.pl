...
#
# Release 1.4.2a (2015-03-11) Mikael Cam (mikael (at) nateis.com)
...
case "wtp" { ($state, $string) = get_wtp_state("%"); }
...
my $string_errors = "";
my $downwtp = "";
my %entitystate = ( '1' => 'down', # Enumeration for the wtp up/down states
'2' => 'up' );
$state = "OK";
...
$string = "$state - $wtpoffline offline WiFi access point(s) over $wtpcount found : ".(sprintf("%.2f",$value))." $UOM : ".$downwtp;
}
else
{
$string = "No wtp configured.";
}
