...
use lib "/usr/lib/perl5/site_perl/5.8.8/NetApp"; 		# this has to be adjusted!
...
# prepare netapp info hash 
my %netapp = get_netapp_info();

# commands
if ( $command =~ /check-volume/ ) {
	%exit_hash = check_volume();
} elsif ( $command =~ /check-snapmirror/ ) {
	%exit_hash = check_snapmirror();
} elsif ( $command =~ /check-lun/ ) {
	%exit_hash = check_lun();
} elsif ( $command =~ /check-aggr/ ) {
	%exit_hash = check_aggr();
} elsif ( $command =~ /check-cluster/ ) {
	%exit_hash = check_cluster();
...
sub init_netapp {
	...
	if ($dossl) {
			...
			if (ref ($response) eq "NaElement" && $response->results_errno != 0) {
					...
					$np->nagios_exit(CRITICAL, "Unable to set HTTPS transport $r");
			}
	}
	...
	if (ref ($response) eq "NaElement") {
			if ($response->results_errno != 0) {
					my $r = $response->results_reason();
					$np->nagios_exit(CRITICAL, "CRITICAL: Unable to set server type $r");
			}
	}
	...
	if($timeout > 0) {
		...
		$np->nagios_exit(CRITICAL, "Invalid value for connection timeout. Connection timeout value should be greater than 0");
	}
}
...
sub check_snapmirror {
	...
	foreach $snap (@result){
		...
		next if ($snapmirror_dst_loc[0] ne $netapp{name});
...
sub byteconv($) {
	my $c = shift;
	$c >= 1073741824 ? sprintf("%0.2f TB", $c/1073741824)
		: $c >= 1048576 ? sprintf("%0.2f GB", $c/1048576)
		: $c >= 1024 ? sprintf("%0.2f MB", $c/1024)
		: $c . " B";
}
